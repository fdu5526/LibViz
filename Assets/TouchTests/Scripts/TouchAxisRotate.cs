...
public class TouchAxisRotate: BBSimpleTouchableObject {
...
//		if (movement.sqrMagnitude > 0.01) {
		...
		if (allowRotateX) {	
			pivot.transform.Rotate(movement.x * 2, 0, 0,Space.World);
		}
		if (allowRotateY) {	
			pivot.transform.Rotate(0, movement.y * 2, 0,Space.World);
		}
			this.endPivot();			
//		}
	}
