...
public class TouchAxisRemoteRotate: BBSimpleTouchableObject {
...
//		if (movement.sqrMagnitude > 0.01) {
		...
		if (allowRotateX) {	
			...
			PanoReference.transform.Rotate(movement.y * 5, 0, 0,Space.World);
		}
		if (allowRotateY) {	
			pivot.transform.Translate(movement.x, 0, 0,Space.World);
			PanoReference.transform.Rotate(0, movement.x * 20, 0,Space.World);
		}
			this.endPivot();			
//		}
	}
